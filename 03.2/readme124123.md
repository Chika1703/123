...
есть проблема в том, что в день есть ограничение на создание айпишников (10шт в день), а т.к я выполняю команды terraform destroy , то они очень быстро расходуются =(
но есть и другие проблемы связанные с этим сервисом. я не могу использовать аргумент dynamic block , т.к такого аргумента попросту не существует. Этому есть обьяснение: в файле disk_vm.tf  есть блок кода: resource "twc_server_disk" "additional_disks" {
  depends_on        = [twc_server.storage]
  count             = 3
  source_server_id  = twc_server.storage.id
  size              = 10240
}
 она используется после создания сервера storage и сразу же привязывается к нему с помощью аргумента   строчки source_server_id  = twc_server.storage.id
насчет dns, а именно переменная fqdn . технический днс нельзя создавать с помощью аргументов, как я понял, их можно создавать и привязывать к серверам только через веб интерфейс. если мне нужно указать днс в ansible инвентаре , то я могу это сделать через костыль/механически (это не будет соответствовать заданию)
пример:
ansible.tf будет выглядеть так:
resource "local_file" "ansible_inventory" {
  content = templatefile("${path.module}/inventory.tpl", {
    webservers_ips = twc_floating_ip.web_ips[*].ip,
...
[webservers] 
%{ for i, dns in webservers_dns ~} # костыль
web-${i + 1} ansible_host=${dns} # костыль
%{ endfor ~} # костыль
...
[databases] 
%{ for i, dns in database_dns ~} # костыль
db-${i + 1} ansible_host=${dns} # костыль
%{ endfor ~} # костыль
...
storage ansible_host=${storage_ip}
по поводу depends_on это моя ошибка, я невнимательно прочел задание, и хочу предложить немного исправить Задание 2 , т.к оно непонятно оформлено после кода

решение этой проблемы?:

файл count-vm.tf по создание серверов web
resource "twc_floating_ip" "web_ips" {
  count             = 2
  availability_zone = var.availability_zone
}
...
так же в задание 2 я не использую Используйте функцию file в local-переменной для считывания ключа ~/.ssh/id_rsa.pub и его последующего использования в блоке metadata а делаю это через секретный файл
